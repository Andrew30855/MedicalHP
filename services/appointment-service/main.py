    ...
    with db() as conn:
        rows = conn.execute(
            ...
            SELECT a.id, a.patient_id, p.full_name AS patient_name, a.doctor_id, d.full_name AS doctor_name,
                   d.specialty, a.slot_id, s.starts_at, a.status, a.idempotency_key, a.trace_id, a.created_at
            FROM appointments a
        ...
        with conn.transaction():
            ...
            if not inserted:
                existing = conn.execute(
                    "SELECT id, patient_id, doctor_id, slot_id, status, amount_cents, idempotency_key, trace_id, created_at FROM appointments WHERE idempotency_key = %s",
                    (idem,),
                ).fetchone()
